...
public class IridiumUser<T extends Team> extends DatabaseObject {
    ...
    private IridiumUserProfile<T> profileCache;

    @SuppressWarnings("unchecked")
    public IridiumUserProfile<T> getActiveProfile() {
        if (profileCache != null)
            return profileCache;
        Optional<IridiumUserProfile<T>> maybeProfile = IridiumTeams.Singleton.get(Team.class, IridiumUser.class)
                .getUserManager().getUserProfile(profile);
        if (!maybeProfile.isPresent()) {
            List<IridiumUserProfile<T>> profiles = IridiumTeams.Singleton.get(Team.class, IridiumUser.class)
                    .getUserManager().getUserProfiles(this);
            if (profiles.size() > 0) {
                this.profile = profiles.get(0).getId();
                this.setChanged(true);
                return profiles.get(0);
            } else {
                IridiumUserProfile<T> p = IridiumTeams.Singleton.get(Team.class, IridiumUser.class).getUserManager()
                        .createUserProfile(this, "default");
    ...
    public boolean canFly(IridiumTeams<T, ?> iridiumTeams) {
        ...
        if (player.hasPermission(iridiumTeams.getCommands().flyCommand.permission))
            return true;
        if (isBypassing())
            return true;
    ...
    public void applyPotionEffects(IridiumTeams<T, ?> iridiumTeams) {
        ...
        if (player == null)
            return;
        iridiumTeams.getTeamManager().getTeamViaLocation(player.getLocation())
                .ifPresent(t -> applyPotionEffects(iridiumTeams, t));
        iridiumTeams.getTeamManager().getTeamViaID(getActiveProfile().getTeamID())
                .ifPresent(t -> applyPotionEffects(iridiumTeams, t));
    }
    ...
    public void applyPotionEffects(IridiumTeams<T, ?> iridiumTeams, T team) {
        ...
        for (Map.Entry<String, Enhancement<?>> enhancement : iridiumTeams.getEnhancementList().entrySet()) {
            TeamEnhancement teamEnhancement = iridiumTeams.getTeamManager().getTeamEnhancement(team,
                    enhancement.getKey());
            if (!teamEnhancement.isActive(enhancement.getValue().type))
                continue;
            ...
            if (enhancementData instanceof PotionEnhancementData) {
                ...
                if (!canApply(iridiumTeams, team, potionEnhancementData.enhancementAffectsType))
                    continue;
        ...
        for (Map.Entry<PotionEffectType, Integer> potionEffectType : potionEffects.entrySet()) {
            ...
            if (potionEffect.isPresent()) {
                if (potionEffect.get().getAmplifier() <= potionEffectType.getValue()
                        && potionEffect.get().getDuration() <= duration * 20) {
    ...
    public boolean canApply(IridiumTeams<T, ?> iridiumTeams, T team,
            List<EnhancementAffectsType> enhancementAffectsTypes) {
        Player player = getPlayer();
        if (player == null)
            return false;
        int teamLocationID = iridiumTeams.getTeamManager().getTeamViaLocation(player.getLocation()).map(T::getId)
                .orElse(0);
        for (EnhancementAffectsType enhancementAffectsType : enhancementAffectsTypes) {
            ...
            if (enhancementAffectsType == EnhancementAffectsType.MEMBERS_ANYWHERE
                    && team.getId() == getActiveProfile().getTeamID()) {
                return true;
            }
            if (enhancementAffectsType == EnhancementAffectsType.MEMBERS_IN_TERRITORY
                    && team.getId() == getActiveProfile().getTeamID() && team.getId() == teamLocationID) {
                return true;
            }
        }
        return false;
